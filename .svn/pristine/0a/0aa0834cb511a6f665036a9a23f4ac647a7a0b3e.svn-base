			...
			<div class="main-container-inner">
				...
				<div class="main-content">
					...
					<div class="page-content">
						<small>
							<h4 class="weight">学员姓名:<?php echo $student_info['student_name'];?>
								<a class="btn btn-xs btn-info" role="button" href="<?php echo site_url(module_folder(2).'/student_course/index/'.$student_info['student_id']);?>">课程列表</a>
							</h4>
						</small>
									...
									<div class="col-xs-12">
										<?php foreach ($list as $key => $value) { ?>
											<div style="border:1px dashed #999; padding:20px; margin-bottom:20px;">
												...
												<?php }?>
												<?php
													$str = $value['course_info'];
													$str .= '<br />';
													$str .= '缴费类型：'.$value['payment_type_name'];
													$str .= '<br />';
													$str .= '特殊情况备注：'.$value['special_payment_remark'];
													$str .= '<br />';
													$str .= '应缴学费：'.$value['study_expense'].' 元';
													$str .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
													$str .= '已缴学费是：'.$value['already_payment'].' 元';
													$str .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';

													if($value['position_total']){
														$str .= '(其中：定位费：'.$value['position_total'].' 元）';
														$str .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
													}
												...
												<?php if($value['is_fail'] == 1){?>
												<a class="btn btn-xs btn-warning course_info right" href="<?php echo site_url(module_folder(2).'/student_course/edit/'.$value['student_id'].'/'.$value['repayment_id']);?>">修改报读课程</a>
												<?php }?>
											<?php if($value['payment_type_id'] == 1){ ?>
												<!-- 一次性付款 -->
												<?php if($value['is_fail'] == 1){?>
												<button data-toggle="modal" data-target="#disposable_pay" type="button" data-paydisid="<?php echo $value['repayment_id'];?>"  class="btn btn-xs btn-primary right">添加学费</button>
												<?php }?>
												...
												<table id="sample-table-1" class="table table-striped table-bordered table-hover table-condensed">
													<thead>
														<tr>
															...
															<th class="center hidden-480">应缴学费总数(元)</th>
															<th class="center hidden-480">学费说明</th>
											...
											<div class="table-responsive">
												...
												<table id="sample-table-1" class="table table-striped table-bordered table-hover">
													<thead>	
														<tr>
															<th class="center">序号</th>
															<th class="center">应缴学费总数(元)</th>
															<th class="center">学费说明</th>
															<th class="center">应缴费金额(元)</th>
															<th class="center">应缴学费日期</th>
											...
											<?php }else if($value['payment_type_id'] == 3){ ?>
												<!-- 先就业后付款(包吃住) -->
												<?php if($value['is_fail'] == 1){?>
												<button data-toggle="modal" data-target="#apply_payment" type="button" data-applyid="<?php echo $value['repayment_id'];?>"  class="btn btn-xs btn-primary right">修改申请情况</button>
											...
											<?php }else if($value['payment_type_id'] == 5){ ?>
												<!-- 先就业后付款(工资方案) -->
												<?php if($value['is_fail'] == 1){?>
												<button data-toggle="modal" data-target="#apply_payment" type="button" data-applyid="<?php echo $value['repayment_id'];?>"  class="btn btn-xs btn-primary right">修改申请情况</button>
											...
											<?php }else{ ?>	
												...
												<table id="sample-table-1" class="table table-striped table-bordered table-hover">
													...
													<tbody>
														<?php
														 foreach($value['refund_loan_time'] as $k=>$item){ 
															if($value['repayment_id'] == $item['repayment_id']){
															?>
														<tr>
															<td class="center">
...
<![endif]-->
		...
		<!--模态框（弹出 咨询者的详细信息信息）-->
		<div aria-hidden="true" aria-labelledby="youModalLabel" role="dialog" tabindex="-1" class="modal fade in" id="life_help_info" style="display: none;">
		  	<div class="modal-dialog" style="width:500px;padding-top:100px;">
		        <div class="modal-content">
		         	<div class="modal-header">
		            	<h4 id="youModalLabel" class="modal-title">生活补贴</h4>
		          	</div>
		...
		<div class="modal fade" id="disposable_pay" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
		  <div class="modal-dialog">
		    <div class="modal-content">
		      ...
		      <form action="<?php echo site_url(module_folder(2).'/student_payment/installDisposable');?>" method="post"  id="installmentsEdit">
		...
		<div class="modal fade" id="editmoney_content" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"  style="display: none;">
		  <div class="modal-dialog">
		    <div class="modal-content">
		      ...
		      <form action="<?php echo site_url(module_folder(2).'/student_payment/installmentsEdit');?>" method="post" id="installmentAjax">
		...
		<!-- 先就业后付款弹出框	  -->
		<div class="modal fade" id="apply_payment" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"  style="display: none;">
		  <div class="modal-dialog">
		    <div class="modal-content">
		      <div class="modal-header">
		        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
		      ...
		      <form action="<?php echo site_url(module_folder(2).'/student_payment/installapplyEdit');?>" method="post" id="installmentAjax">
		      <input type="hidden" id="data-payid" name="data_payid" value="" />
		      <div class="modal-body" id="applymodal-body">
		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body" id="modal-combody">
		        <div id="delete_paymoney" style="display:none;margin-bottom:10px;">请填写退费金额：<input type="text" value="" name="delete_paymoney" placeholder="请填写退费金额" id="payment_money" /> 元</div>
		      	...
		      	<div>请确认删除日期：<input type="text" class="date-picker" data-target="#dateShow" data-date-format="yyyy/mm/dd" value="" name="delete_paytime" placeholder="请确认删除日期"></div>
		      </div>	
		      <div class="modal-footer">
		        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
		...
		<!-- 先就业后付款输入框 -->
		<textarea name="loans" style="display:none;">
			<tr>
		 		<td><input type="text" data-target="#dateShow" class="col-xs-12" data-date-format="yyyy/mm/dd" name="add_payment_time[]" placeholder="补贴日期" /></td>
  		 		<td><input type="text" name="add_payment_money[]" class="col-xs-12" placeholder="补贴金额" /></td>
  		 		<td><input type="text" data-target="#dateShow" class="col-xs-12" data-date-format="yyyy/mm/dd" name="add_remind_time[]" placeholder="提醒时间" /></td>
  		 		<td><input type="text" name="add_payment_desc[]" class="col-xs-12" placeholder="补贴说明" /></td>
  		 		<td>
  		 			<button type="button" onclick="wdcrm.removeInput(this,2);" class="btn spinner-down btn-xs btn-danger">
						<i class="icon-minus smaller-75"></i>
					</button>
		...
		<script type="text/javascript">
			jQuery(function($) {
				...
				$('.icon-edit').click(function(){
					window.location.href="<?php echo site_url(module_folder(2).'/advisory/edit');?>";
				});
		...
		jQuery(function($){
				...
				$('button[data-target="#disposable_pay"]').click(function(){
					...
					var student_id = "<?php echo $student_info['student_id'];?>";
					$.ajax({
				        type: "POST",
				        url: "<?php echo site_url(module_folder(2).'/student_payment/disposableAjax');?>",
				        data: "id="+paydisid+"&student_id="+student_id,
				        dataType:'json',
				        success: function(res){
				...
				$('button[data-target="#editmoney_content"]').click(function(){
					...
					$.ajax({
				        type: "POST",
				        url: "<?php echo site_url(module_folder(2).'/student_payment/installmentAjax');?>",
				        data: "id="+payid+"&student_id="+student_id,
				        dataType:'json',
				        success: function(res){
				       		if(res.status==0){return ;}
				...
				$('button[data-target="#apply_payment"]').click(function(){
				
					var applyid=parseInt($(this).attr("data-applyid"));
					$('#data-payid').val(applyid);

					var student_id = <?php echo $student_info['student_id'];?>;
					$.ajax({
				        type: "POST",
				        url: "<?php echo site_url(module_folder(2).'/student_payment/applyAjax');?>",
				        data: "id="+applyid+"&student_id="+student_id,
				        dataType:'json',
				        success: function(res){
				        	if(res.status==0){return ;}	
				       		$('#applymodal-body').html(res.data);
				...
				$('button[data-target="#edit_pay_one"]').click(function(){
					...
					$.ajax({
				        type: "POST",
				        url: "<?php echo site_url(module_folder(2).'/student_payment/payinfoAjax');?>",
				        data: "id="+applyid,
				        dataType:'json',
				        success: function(res){
				        	if(res.status==0){return ;}
				...
				$('#delete_submit').click(function () {		
					...
					if(action_type==1){
						var url = "<?php echo site_url(module_folder(2).'/student_payment/delete');?>";
					}else if(action_type==2){
						if(delete_paymoney==''){
							alert("请填写退费金额");
							return false;
						}else{
							var url = "<?php echo site_url(module_folder(2).'/student_payment/delete_paycord');?>";
						}				
					}
					$.ajax({
				        type: "POST",
				...
				$('button[data-target="#delete_pay2"]').click( function() {
					...
					bootbox.confirm("你确定删除该缴费记录吗?", function(result) {
						if(result) {
							$.ajax({
						        type: "POST",
						        url: "<?php echo site_url(module_folder(2).'/student_payment/delete_pay');?>",
						        data: "pay_id="+pay_id,
						        dataType:'json',
						        success: function(res){
						        	if(res.data == 1){
