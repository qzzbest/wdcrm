						...
						<div class="row">
							<div class="col-xs-12">
								...
								<form class="form-horizontal" role="form" method="post" action="">
									<input type="hidden" name="payment_time_nums" value="<?php echo $payment_time_nums;?>" />
									<input type="hidden" name="second_payment_time" value="<?php echo $second_payment_time;?>" />
									<input type="hidden" name="old_employee_id" value="<?php echo $student['employee_id'];?>" />
										...
										<div class="col-sm-3">
											<select class="form-control" name="employee_id">
												...
												<?php foreach($teach as $item){?>
													<option <?php 
														if ($student['employee_id']==$item['employee_id']) {
															echo 'selected';
														}
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input type="text" class="date-picker" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="sign_up_date" value="<?php echo date('Y/m/d',$student['sign_up_date']);?>" placeholder="缴费日期" style="width:296px;" />
										</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-9" style="width:312px;">
											<input type="text" required="" value="<?php echo $student['student_name'];?>" name="student_name" id="form-field-1" placeholder="咨询者姓名" style="width:296px;" class="col-xs-10 col-sm-5" />
										</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input type="text" name="student_other_contacts" placeholder="其他联系方式" value="<?php echo $student['student_other_contacts'];?>" class="col-xs-10 col-sm-10" />				
										</div>
									</div>
										...
										<div class="col-sm-9">
											...
											<div class="radio">
													<label style="padding-right:20px;">
														<input name="sex" <?php if ($student['student_sex']==1) {
															echo "checked";
														} ?> type="radio" value="1" class="ace" />
														<span class="lbl"> 男</span>
													</label>
													<label>
														<input name="sex" <?php if ($student['student_sex']==2) {
															echo "checked";
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<textarea class="form-control" name="student_otherinfo" id="form-field-8" placeholder="这里可以填写关于咨询者备注信息"><?php echo $student['student_otherinfo'];?></textarea>
										</div>
										<div class="col-sm-5">&nbsp;</div>
									</div>
									<div class="space-4"></div>
									...
									<div style="display:<?php
										...
										<div class="col-sm-3"></div>
										<div class="col-sm-4">
											<input type="text" name="student_education_other" value="<?php echo $student['student_education'];?>" placeholder="请输入学历" class="col-xs-10 col-sm-12" />
										</div>
										<div class="col-sm-5"></div>
									</div>
									<div class="space-4"></div>
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" name="student_school" id="form-field-6" value="<?php echo $student['student_school'];?>" placeholder="咨询者毕业院校"  class="col-xs-10 col-sm-5" />
										</div>
									</div>
									<div class="space-4"></div>
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" name="student_specialty" id="form-field-7" value="<?php echo $student['student_specialty'];?>" placeholder="咨询者就读专业" class="col-xs-10 col-sm-5" />
										</div>
									</div>
	 		...
	 		jQuery(function($) {
	 			...
	 			checkInput.prototype = {
					...
					add_blur2:function(){
						var _this=this;
						$('input[name="'+name+'"]').unbind();
						$('input[name="'+name+'"]').bind('blur',function(){_this.blur_ajax2(this);});
					},
					...
					blur_ajax2:function(obj){

						var k=obj.name;
						var v= wdcrm.strtotime($.trim(obj.value));

						var date = "<?php echo $student['sign_up_date'];?>";
						var second_payment_time = $('input[name="second_payment_time"]').val();
						var payment_time_nums = $('input[name="payment_time_nums"]').val();

						if( v != ''){
							if( v != date ){
								if( v > second_payment_time && payment_time_nums == 'more'){

									$(obj).parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline">您选择报名时间不是最早的缴费时间！（请到缴费记录更改）</div>');
								}else{
									
									$(obj).parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline"></div>');
								}
							}else{
								$(obj).parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline"></div>');
							}
					 		...
					 		$(obj).parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline">请输入报名时间！</div>');
	 			...
	 			new checkInput('sign_up_date',2).add_blur2(); 
				...
				//咨询者咨询形式,选择其他的时候出现一个输入框
		 		...
		 		$('.form-horizontal').submit(function () {
					...
					//检查报名时间
					var sign_up_date = $('input[name="sign_up_date"]').val();
					sign_up_date= wdcrm.strtotime(sign_up_date);
					...
					if( sign_up_date != ''){
						if( sign_up_date != date ){
							if( sign_up_date > second_payment_time && payment_time_nums == 'more'){
								$('input[name="sign_up_date"]').parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline">您选择报名时间不是最早的缴费时间！（请到缴费记录更改）</div>');
								return false;
							}
							
						}else{
							$('input[name="sign_up_date"]').parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline"></div>');
						}
						
					}else{

				 		$('input[name="sign_up_date"]').parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline">请输入报名时间！</div>');
				 		return false;
	 				...
	 				if (a=='-1') {
	 					$('select[name="consultant_consultate_id"]').parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline">请选择咨询形式</div>');
	 					return false;
	 				}

	 					a= $('select[name="consultant_channel_id"] option:selected').val();
	 				if (a=='-1') {
	 					$('select[name="consultant_channel_id"]').parent().next().html('<div style="color: rgb(209, 110, 108);" class="help-block col-xs-12 col-sm-reset inline">请选择咨询者渠道</div>');
	 					return false;
	 				}
