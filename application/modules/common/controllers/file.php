...
class Common_file_module extends CI_Module {
	...
	public function index()
	{
		...
		$list = $this->upload_files->select_index($start,$limit,$file_status);
		$count = $this->upload_files->select_index_count($file_status);
		//未下载总数
		$data['nodown_count'] = $this->main_data_model->count(array('file_status'=>1));
		//已下载总数
		$data['down_count'] = $this->main_data_model->count(array('file_status'=>2));
		...
		$data['tiao']=$config['base_url']=site_url(module_folder(5)."/file/index/".$file_status.'?');
	...
	{	
		$url[0]=array('文件列表', site_url(module_folder(5).'/file/index/'));
		...
		$url[1]=array('文件分页',site_url(module_folder(5).'/file/index/'.$file_status.'?per_page='.$per_page));
