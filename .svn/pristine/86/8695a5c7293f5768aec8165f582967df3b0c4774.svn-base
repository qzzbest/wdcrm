 ...
 * 学生表模型
 */
class Advisory_client_model extends CI_Model 
{
	private $t;#表名
	...
	{
		parent::__construct();
		$this->t=$this->db->dbprefix('client');
	...
	public function select_index($start,$limit,$key,$like='',$start_time,$end_time,$order=0)
	{
		...
		$field=$this->t.'.student_id,'.$this->t.'.student_name,'.$this->t.'.student_sex,'.$this->t.'.sign_up_date,'.$this->t.'.consultant_id,'.$this->t.'.employee_id,'.$this->t.'.show_status';
	...
	{	
		...
		$this->db->select($this->t.'.student_id,'.$this->t.'.student_sex,'.$this->t.'.student_name,'.$this->t.'.consultant_id,'.$this->t.'.sign_up_date,'.$this->t.'.employee_id,'.$this->t.'.show_status')
			     ->from($this->t)
	...
	public function select_contact_like($data,$start,$limit)
	{
		...
		$this->db->select($this->t.'.student_id,'.$this->t.'.student_name,'.$this->t.'.student_sex,'.$this->t.'.consultant_id,'.$this->t.'.sign_up_date,'.$this->t.'.employee_id,'.$this->t.'.show_status')
	...
	public function select_record_time($start,$limit,$start_time,$end_time,$teach_id='')
	{
		...
		if($this->p==0){//如果是普通的员工
			$sql="SELECT r.`consultant_id`, r.`consultant_record_time`, c.* FROM `crm_consultant_record` r left join crm_student c on r.`consultant_id`=c.`consultant_id` WHERE c.`employee_id`={$id} AND {$w} group by r.`consultant_id` LIMIT $start,$limit";
		}else{
			$sql="SELECT r.`consultant_id`, r.`consultant_record_time`, c.* FROM `crm_consultant_record` r left join crm_student c on r.`consultant_id`=c.`consultant_id` WHERE {$w} group by r.`consultant_id` LIMIT $start,$limit";
	...
	public function select_record_time_count($start_time,$end_time,$teach_id='')
	{
		...
		if($this->p==0){//如果是普通的员工
			$sql="SELECT COUNT( DISTINCT r.`consultant_id`) as l FROM `crm_consultant_record` r left join crm_student c on r.`consultant_id`=c.`consultant_id` WHERE c.`employee_id`={$id} AND {$w}";
		}else{
			$sql="SELECT COUNT( DISTINCT r.`consultant_id`) as l FROM `crm_consultant_record` r left join crm_student c on r.`consultant_id`=c.`consultant_id` WHERE {$w}";
		}
