...
<html lang="en">
	...
	<body>
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<form class="form-horizontal" action="<?php echo site_url(module_folder(2).'/client_project/edit');?>" method="post" id="setClient">
									<div class="form-group">
						          	...
						          	<input type="hidden" name="repayment_id" value="<?php echo $id;?>" />
						          	<input type="hidden" name="old_payment_type_id" value="<?php echo $list[0]['payment_type_id'];?>" />
									<input type="hidden" name="old_id" value="<?php if(!empty($min_info)){echo $min_info[0]['id'];}?>" />
						          	...
						          	<table cellpadding="5px" style="margin: 20px 10px 0px;">
										<tr>
											...
											<td width="490px">
												<input type="text" name="project_name" placeholder="请输入项目名称" value="<?php echo $project_info['project_name']?>" id="project_name" type-data="false" style=" float:left;" />
										...
										<tr>
											...
											<td width="490px">
												<input type="text" name="project_url" placeholder="请输入项目参考网址" value="<?php echo $project_info['project_url']?>" id="project_url" type-data="false" style=" float:left;" />
												<div class="col-sm-5" style="float:left;"></div>
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												<span style=" float:left;">
													<input type="text" name="project_total_money" id="project_total_money" value="<?php echo $list[0]['study_expense'];?>" placeholder="请输入项目总费用" /> 元
												</span>
												<div class="col-sm-5" style=" float:left;"></div>
											</td>
										</tr>
										...
										<tr>
											...
											<td width="490px">
												<input type="text" class="date-picker" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="project_payment_time" value="<?php if(!empty($min_info)){echo date('Y/m/d',$min_info[0]['already_paytime']);}else{echo date('Y/m/d');}?>" placeholder="缴费日期" />
											</td>
										</tr>
										...
										<tr>
											...
											<td width="490px">
												<span style=" float:left;">
													<input type="text" name="project_already_total" placeholder="请输入费用" value="<?php if(!empty($min_info) && $min_info[0]['payment_money'] != 0){echo $min_info[0]['payment_money'];}?>" id="project_already_total" /> 元
												</span>
												<div class="col-sm-5" style="float:left;"></div>
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												...
												foreach ($payment_type_info as $key => $value) { 
														if($list[0]['payment_type_id'] == $value['payment_type_id']){
													?>
													<label style="padding-right:10px;">
														<input name="payment_type_id" checked="checked" data-target="payment_type_id" type="radio" value="<?php echo $value['payment_type_id'];?>" class="ace" />
														<span class="lbl"> <?php echo $value['payment_type_name'];?></span>
													</label>
												<?php }else if(in_array($value['payment_type_id'],$payment_type_arr)){ ?>
													<label style="padding-right:10px;">
									...
									<div class="modal-body" style="padding:0px;padding-bottom:0px;" data-target="projectChange" id="projectChange">
										<table style="display:none; margin: 0 auto;" id="payment_two">
												...
												<?php
												if(!empty($refund_loan_info) && $list[0]['payment_type_id'] == 2){ ?>
												<tr>
												...
												<?php foreach ($refund_loan_info as $key => $value) {
													...
													<?php if($value['payment_status'] == 1){?>
												 	<tr>
												 		<td><input type="text" class="date-picker" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="pro_payment_time[]" placeholder="应缴费日期" value="<?php if(!empty($value['payment_time'])){echo date('Y/m/d',$value['payment_time']);;}?>" disabled="disabled" /></td>
												 		<td><input type="text" name="pro_payment_money[]" placeholder="应缴费金额" value="<?php echo $value['payment_money'];?>" disabled="disabled" />元</td>
												 		<td><input type="text" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="pro_remind_time[]" placeholder="提醒时间" value="<?php if(!empty($value['remind_time'])){echo date('Y/m/d',$value['remind_time']);}?>" disabled="disabled" /></td>
												 		<td><input type="text" name="pro_payment_desc[]" placeholder="费用说明" value="<?php echo $value['payment_desc'];?>" disabled="disabled" /></td>
												 		<td>
												 			已完成
														</td>
												 	</tr>
												 	<?php }else{?>
													<tr>
												 		<td><input type="text" class="date-picker" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="update_pro_payment_time[<?php echo  $value['id'];?>]" placeholder="应缴费日期" value="<?php if(!empty($value['payment_time'])){echo date('Y/m/d',$value['payment_time']);;}?>" /></td>
												 		<td><input type="text" name="update_pro_payment_money[<?php echo  $value['id'];?>]" placeholder="应缴费金额" value="<?php echo $value['payment_money'];?>" />元</td>
												 		<td><input type="text" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="update_pro_remind_time[<?php echo  $value['id'];?>]" placeholder="提醒时间" value="<?php if(!empty($value['remind_time'])){echo date('Y/m/d',$value['remind_time']);}?>" /></td>
												 		<td><input type="text" name="update_pro_payment_desc[<?php echo  $value['id'];?>]" placeholder="费用说明" value="<?php echo $value['payment_desc'];?>" /></td>
												 		<td>
									...
									<table cellpadding="5px" style="margin: 20px 10px 0px;">
										<tr>
											...
											<td width="300px">
												<textarea style="width: 500px;height: 119px;" name="project_payment_remark" class="form-control" id="form-field-8" placeholder="针对客户的情况备注信息"><?php echo $project_info['project_remark'];?></textarea>
											</td>
										</tr>
									</table>
		...
		<textarea name="proinstallments" cols="30" rows="10" style="display:none;">
			<tr>
		 		<td><input type="text" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="add_pro_payment_time[]" placeholder="应缴费日期" /></td>
  		 		<td><input type="text" name="add_pro_payment_money[]" placeholder="应缴费金额" />元</td>
  		 		<td><input type="text" data-target="#dateShow" data-date-format="yyyy/mm/dd" name="add_pro_remind_time[]" placeholder="提醒时间" /></td>
  		 		<td><input type="text" name="add_pro_payment_desc[]" placeholder="费用说明" /></td>
  		 		<td>
...
<![endif]-->
		...
		<script type="text/javascript">
			...
			jQuery(function($){	
				...
				//默认显示的缴费类型
				var payment_type_id = "<?php echo $list[0]['payment_type_id'];?>";
				if(payment_type_id == 2){
					var z = $('div[data-target="projectChange"]').find('table').eq(0);
				}
			...
			jQuery(function($){	
				...
				$('.del_payment').click(function(){
					...
					$.ajax({
				        type: "POST",
				        url: "<?php echo site_url(module_folder(2).'/client_project/deletePayRef');?>",
				        data: "pid="+pid,
				        dataType:'json',
