...
<html lang="en">
	...
	<body>
				...
				<div class="main-content">
					...
					<div class="page-content">
						<div class="page-header">
							<small>
								<a class="btn btn-xs btn-info" href="<?php echo site_url(module_folder(2).'/remind/add');?>">添加咨询提醒</a>
							</small>
						</div>
						<div class="row">
							<div class="col-xs-12">
											...
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												<?php foreach ($list as $key=>$value) {
													...
													<tr>
														...
														<td><?php if(isset($value['consultant']['consultant_name'])){echo "姓名: ".$value['consultant']['consultant_name']."<br />手机号码: ".implode(',',$value['phone'])."<br />QQ号码: ".implode(',',$value['qq']);} ?></td>
														<td>
														...
														<td>									
															<a class="btn btn-xs btn-info" href="<?php echo site_url(module_folder(2).'/remind/edit/'.$value['time_remind_id']); ?>" role="button" data-rel="tooltip" title="修改">修改</a>

															<?php if($value['consultant_id']!=0){?>
															<a type="button" href="<?php echo site_url(module_folder(2).'/advisory/index/index/0?consultant_id='.$value['consultant_id']);?>" target="_blank" class="btn btn-xs btn-primary delay_remind" title="咨询者信息">咨询者信息</a>
															<?php }?>
		...
		<script type="text/javascript">
			(function(){
				//如果是已上门的，给背景颜色
				var data='<?php echo $remind_id;?>';
				var remind_id=JSON.parse(data);
				...
				$('input[name="id[]"]').each(function(){
					var res=wdcrm.in_array(this.value,remind_id);
			...
			jQuery(function($) {
				...
				$(".one_delete").on(ace.click_event, function() {
					...
					bootbox.confirm("你确定删除提醒吗?", function(result) {
						if(result) {
							$.ajax({
						        type: "POST",
						        url: "<?php echo site_url(module_folder(2).'/remind/deleteRemind');?>",
						        data: "id="+data_del,
						        dataType:'json',
				...
				$(".all_ignore").on(ace.click_event, function() {
					$("#all").attr("action","<?php echo site_url(module_folder(2).'/remind/ignore');?>");
				...
				$(".all_edit").click(function() {
					$("#all").attr("action","<?php echo site_url(module_folder(2).'/remind/allEdit');?>");
