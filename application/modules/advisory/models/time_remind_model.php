...
class Advisory_time_remind_model extends CI_Model 
{
	private $t;#咨询者的咨询形式表
	...
	public function select_index($employee_id,$start,$limit,$start_time,$end_time)
	{
		...
		// 		 ->order_by("FROM_UNIXTIME(time_remind_time)","desc")
		// 		 ->order_by("is_important","desc")
		// 		 ->order_by("is_set_view","desc")
		...
		if($start_time != '' and $end_time==''){
       	...
       	$sql = "SELECT * FROM `crm_time_remind` WHERE $where order by FROM_UNIXTIME(`time_remind_time`,'%Y%m%d') asc, `is_important` desc, `is_set_view` desc, `time_remind_time` asc, `time_remind_status` desc $limit";
